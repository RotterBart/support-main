...
@app.route('/dnstrings_delete', methods=['GET', 'POST'])
def dnstrings_delete():
    with pg.connect(
        host='10.10.2.209',
        dbname='DN_Container',
        user='postgres',
        ...
        if conn:
            with conn.cursor() as cursor:
                cursor.execute("UPDATE dnstring SET active = FALSE WHERE active = TRUE;")
                conn.commit()
                return redirect(url_for('dnstrings'))
...
def get_enterprises():
    ...
    with pg.connect(
        ...
        with odo.cursor() as cursor:
            ...
            new_q = []
            first_part = "SELECT id, name ->> 'ru-RU' as name FROM t_xml_community WHERE type = 'Avrora.Objects.Modules.EMS.Enterprise, Avrora.Objects.Common.Community' AND "
            for itt in q:
                new_q.append(f" name ->> 'ru-RU' ILIKE '%{itt}%'")
                
            query_text = first_part + f""" {" AND ".join(new_q)}""" + "ORDER BY name ASC"
            if 'облакимат' in q:
                query_text = "SELECT id, name ->> 'ru-RU' FROM t_xml_community WHERE id = '3176cd78-0acf-467e-aa51-6cdc7960d0a6'"
...
@app.route("/get_journals/<enterprise>", methods=['GET', 'POST'])
def get_journals(enterprise):
    with pg.connect(
        host=Config.DBHOST,
        ...
        with odo.cursor() as cursor:
            cursor.execute(f"""SELECT id, json_data FROM "{enterprise}".t_docflow_businessobjects WHERE business_object_type = 300 ORDER BY rec_date DESC""")
            journals = cursor.fetchall()
            
            return jsonify({"success": True, "data":journals})

@app.route("/update_journals", methods=['GET', 'POST'])
def update_journals():
    with pg.connect(
        host=Config.DBHOST,
        dbname=Config.DBNAME,
        ...
        with odo.cursor() as cursor:
            ...
            cursor.execute(f"""UPDATE "{enterprise}".t_docflow_businessobjects SET json_data = jsonb_set("json_data", '{{Value}}', '{value}'), rec_date = '{datetime.now()}' WHERE id = '{id}'""")
            odo.commit()
